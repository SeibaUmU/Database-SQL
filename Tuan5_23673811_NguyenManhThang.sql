...
from Customers where ContactTitle like 'O%' 
...
ở thành phố Lyon.*/
select * from Customers where City in ('Lyon') And ContactName Like 'V%'
...
select ProductId, ProductName, Unitprice, UnitsInStock, (Unitprice + UnitsInStock) as Total from Products order by total desc
...
select OrderID, ProductID, Quantity, Unitprice, Discount, (Quantity * unitPrice *(1-Discount)) as TotalLine from [Order Details] where OrderID  in ('10248')

/*
16.Liệt kê danh sách các hóa đơn (orders) có OrderDate được lập trong
tháng 9 năm 1996. Được sắp xếp theo mã khách hàng, cùng mã khách hàng sắp xếp theo ngày lập hóa đơn giảm dần.*/
select * from Orders where YEAR(OrderDate) = 1996 AND MONTH(OrderDate) = 9 ORDER BY CustomerID ASC, OrderID ASC
...
select OrderID, OrderDate, CustomerID, EmployeeID from Orders where OrderDate between '1997-10-01' and '1997-12-31' order by Month(OrderDate)
...
tuần).*/
select OrderID, OrderDate, CustomerID, EmployeeID, datepart(weekday, OrderDate) as WeekDayOfOrdate from Orders WHERE MONTH(OrderDate) = 12 AND YEAR(OrderDate) = 1997 AND DATEPART(WEEKDAY, OrderDate) in (1, 7) order by OrderDate;
--DATEPART() được sử dụng để trả về một phần cụ thể của một giá trị ngày/thời gian (số), WEEKDAY Thứ trong tuần (1=Chủ Nhật, 7=Thứ Bảy)
...
select top 2 EmployeeID, concat(FirstName, ' ', LastName) as EmployeeName, datediff(year, BirthDate, getdate()) as Age from Employees order by Age desc
--DATEDIFF() trong SQL được dùng để tính toán sự chênh lệch giữa hai giá trị thời gian